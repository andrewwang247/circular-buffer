...
private:
	...
	void check_bounds( size_t index ) const {
		if ( index >= num_entries ) {
			std::string message = "Index " + std::to_string(index) + " is out of bounds.";
...
public:
	 ...
	 */
	Circular_Buffer( const T& filler ) : head(0), tail(0), num_entries(N) {
		data.fill(filler);
	}
	 ...
	 * REQUIRES: If std::distance(start, finish) has more elements than N elements,
	 *     takes only the last N elements as expected of a circular buffer.
	 */
	...
	std::vector<T> range() const {
		...
		std::copy( ptr + head, ptr + std::min(head + num_entries, N), std::back_inserter(entries) );
